...
class MonthlyReturnFileUploadWorkItemSpec extends JsonFormatSpec[MonthlyReturnFileUploadWorkItem] {

  override def model: MonthlyReturnFileUploadWorkItem =
    MonthlyReturnFileUploadWorkItem(
  ...
  override def expectedJsonFromWrites: JsValue =
    Json.obj(
      ...
      taxYearFieldName    -> yearOnlyTestTaxYear,
      monthFieldName      -> testMonth,
      referenceFieldName  -> testUploadReference
    )

  override implicit def format: OFormat[MonthlyReturnFileUploadWorkItem] = MonthlyReturnFileUploadWorkItem.format
}
