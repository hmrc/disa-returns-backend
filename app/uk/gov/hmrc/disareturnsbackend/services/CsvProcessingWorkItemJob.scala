...
@Singleton
class CsvProcessingWorkItemJob @Inject() (
  ...
  private implicit val workerExecutionContext: ExecutionContext = {
    actorSystem.dispatchers.lookup("contexts.csv-processing-job")
  }
  ...
  private def runWorker(workerId: Int): Unit = {
    if (stopping.get()) {
      logger.info(s"[CsvProcessingWorkItemJob] Worker $workerId stopping")
      return
    }
    ...
    processNextWorkItem(workerId)
      .recover { case ex =>
        logger.error(
          s"[CsvProcessingWorkItemJob] Worker $workerId failed while processing work items",
          ex
        )
        false
      }
  ...
  private def processNextWorkItem(workerId: Int): Future[Boolean] = {
    ...
    csvProcessingWorkItemRepository.pullOutstanding(now, now).flatMap {
      case Some(workItem) =>
  ...
  private def processWorkItem(
      workerId: Int,
      workItem: WorkItem[CsvProcessingWorkItem]
  ): Future[Boolean] = {
  ...
  private def storeValidatedFile(
    ...
    if (report.isValid) {
      objectStoreConnector.putObject(tempFile, "text/csv").map(_ => ())
    } else {
      Future.successful(())
    }
  }

  private def toParseError(error: CsvValidationError): FileUploadParseError = {
