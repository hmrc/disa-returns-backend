...
@Singleton
class CsvProcessingJobReportWriter @Inject() (
    appConfig: AppConfig
) extends Logging {

  private val reportFile: Path = appConfig.csvProcessingJobReportFile
  ...
  def write(row: CsvProcessingJobReportRow): Unit =
    synchronized {
      try {
        ...
        case ex: Exception =>
          logger.warn(
            s"[CsvProcessingJobReportWriter] Failed to write report row to $reportFile",
            ex
          )
      }
    }

  private def renderRow(row: CsvProcessingJobReportRow): String =
    List(
      row.workItemId,
